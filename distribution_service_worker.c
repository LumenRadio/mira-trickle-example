...
static void distribution_service_udp_callback(
...
static void distribution_service_trickle_callback(
    void *ptr,
    uint8_t supress)
{
    ...
    if (mira_net_get_state() != MIRA_NET_STATE_NOT_ASSOCIATED) {
        if (mira_net_udp_send_to(udp_connection, &distribution_service_dest_addr, DISTRIBUTION_SERVICE_UDP_PORT, buf, 8 + ctx->size) != MIRA_SUCCESS) {
            P_INFO_DS_W("%s: mira_net_udp_send() fail\n", __func__);
        }
    }
}

static int distribution_service_worker_init_net(
    void)
{
    ...
    void *storage)
{
    ...
    trickle_timer_set(
        &ctx->timer,
        distribution_service_trickle_callback,
        ctx);
